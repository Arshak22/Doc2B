        ...
        <div className="StaffPage">
            <div className={"LeftBlockSection" + (darkMode ? ' Dark' : '')}>
                ...
                <div className="userAvatar" style={avatarStyle}>
                        ...
                        <div>
                            {!editMode ?
                                <h2 className={'singleStaffName' + (darkMode ? ' whiteElement' : '')}>{member.first_name} {member.last_name} {member.fathers_name}</h2> :
                                <div className='staffInputSec NameInptSec'>
                                    <div>
                                        <label htmlFor="Name">Անուն</label>
                                        <input type="text" name="Name" id="Name" defaultValue={member.first_name} className={darkMode ? ' darkInpt' : ''}/>
                                    </div>
                                    <div>
                                        <label htmlFor="Surname">Ազգանուն</label>
                                        <input type="text" name="Surname" id="Surname" defaultValue={member.last_name} className={darkMode ? ' darkInpt' : ''}/>
                                    </div>
                                    <div>
                                        <label htmlFor="FatersName">Հայրանուն</label>
                                        <input type="text" name="FatersName" id="FatersName" defaultValue={member.fathers_name} className={darkMode ? ' darkInpt' : ''}/>
                                    </div>
                                </div>
                            }
                            {!editMode ?
                                ...
                                <div className='staffInputSec RoleInptSec'>
                                    ...
                                    <select name="Role" id="Role" defaultValue={member.status} className={darkMode ? ' darkInpt' : ''}>
                ...
                <div className='singleStaffRow'>
                    <div className='PositionDivisionSec'>
                        {!editMode ?
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.position}</h3> :
                            <div className='staffInputSec editStaffInputSec'>
                                <label htmlFor="Position">Պաշտոն</label>
                                <select name="Position" id="Position" defaultValue={member.position} className={darkMode ? ' darkInpt' : ''}>
                        ...
                        {!editMode ?
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.division}</h3> :
                            <div className='staffInputSec editStaffInputSec'>
                                <label htmlFor="Division">Ստորաբաժանում</label>
                                <select name="Division" id="Division" defaultValue={member.division} className={darkMode ? ' darkInpt' : ''}>
                ...
                <div className='singleStaffRow InputsRow'>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        ...
                        {!editMode ?
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.bod}</h3> :
                    ...
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        ...
                        {!editMode ?
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.nationality}</h3> :
                            <input type="text" name="Nationality" id="Nationality" defaultValue={member.nationality} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="Sex">Սեռ</label>
                        {!editMode ? 
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.sex}</h3> :
                            <select name="Sex" id="Sex" defaultValue={member.sex} className={darkMode ? ' darkInpt' : ''}>
                    ...
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        ...
                        {!editMode ? 
                            <a href={'tel:' + member.tel}><h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.tel}</h3></a> :
                            <input type="tel" name="Telephone" id="Telephone" defaultValue={member.tel} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="Email">Էլ․ հասցե</label>
                        {!editMode ? 
                            <a href={'mailto:' + member.email}><h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.email}</h3></a> :
                            <input type="email" name="Email" id="Email" defaultValue={member.email} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                </div>
                <div className='singleStaffRow InputsRow'>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        ...
                        {!editMode ?
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.country}</h3> :
                            <input type="text" name="Country" id="Country" defaultValue={member.country} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="City">Քաղաք</label>
                        {!editMode ? 
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.city}</h3> :
                            <input type="text" name="City" id="City" defaultValue={member.city} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="Address">Հասցե</label>
                        {!editMode ? 
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.address}</h3> :
                            <input type="text" name="Address" id="Address" defaultValue={member.address} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                </div>
                <div className='singleStaffRow InputsRow'>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        ...
                        {!editMode ?
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.passportGivenBy}</h3> :
                            <input type="text" name="PassportGivenBy" id="PassportGivenBy" defaultValue={member.passportGivenBy} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="PassportType">Անձնագրի տեսակը</label>
                        {!editMode ? 
                            <h3 className={(darkMode ? ' whiteElement' : '')}>{member.passportType}</h3> :
                            <input type="text" name="PassportType" id="PassportType" defaultValue={member.passportType} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="PassportNumber">Անձնագրի համարը</label>
                        {!editMode ? 
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.passportNumber}</h3> :
                            <input type="text" name="PassportNumber" id="PassportNumber" defaultValue={member.passportNumber} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="SocialNumber">Սոց․ քարտի համարը</label>
                        {!editMode ? 
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.socialNumber}</h3> :
                            <input type="text" name="SocialNumber" id="SocialNumber" defaultValue={member.socialNumber} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                </div>
                <div className='singleStaffRow InputsRow'>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        ...
                        {!editMode ?
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.weekWorkingDays}</h3> :
                            <input type="number" name="WorkingDaysWeek" id="WorkingDaysWeek" min={1} max={7} defaultValue={member.weekWorkingDays} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="HoursPerWeek">Շաբաթական աշխատաժամանակ</label>
                        {!editMode ? 
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.weekHours}</h3> :
                            <input type="number" name="HoursPerWeek" id="HoursPerWeek" min={1} defaultValue={member.weekHours} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                </div>
                <div className='singleStaffRow InputsRow'>
                    <div className={'staffInputSec DateInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="WorkStartTime">Աշխատանքային օրվա սկիզբ</label>
                        {!editMode ?
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.workStartTime}</h3> :
                            <input type="time" name="WorkingDaysWeek" id="WorkingDaysWeek" defaultValue={member.workStartTime} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec DateInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="WorkEndTime">Աշխատանքային օրվա ավարտ</label>
                        {!editMode ? 
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.workEndTime}</h3> :
                            <input type="time" name="HoursPerWeek" id="HoursPerWeek" defaultValue={member.workEndTime} className={darkMode ? ' darkInpt' : ''}/>
                        }
                    </div>
                    <div className={'staffInputSec DateInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        <label htmlFor="WorkStartDate">Աշխատանքի ընդունման օր</label>
                        {!editMode ?
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.workStartDate}</h3> :
                    ...
                    <div className={'staffInputSec DateInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        ...
                        {!editMode ?
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.workEndDate}</h3> :
                ...
                <div className='singleStaffRow InputsRow'>
                    <div className={'staffInputSec' + (editMode ? ' editStaffInputSec' : '')}>
                        ...
                        {!editMode ? 
                            <h3 className={'numbers' + (darkMode ? ' whiteElement' : '')}>{member.salary} {member.currency}</h3> :
                            <div>
                                <input type="number" name="Salary" id="Salary" min={10000} defaultValue={member.salary} className={darkMode ? ' darkInpt' : ''}/>
                                <select name="Currency" id="Currency" defaultValue={member.currency} className={darkMode ? ' darkInpt' : ''}>
